...
TAG_MAP = {
    "الري": "الري بالطاقة الشمسية",
    "مضخ": "الري بالطاقة الشمسية",
    "زراع": "الري بالطاقة الشمسية",
    "بطاري": "البطاريات والتخزين",
    "تخزين": "البطاريات والتخزين",
    "هجين": "الأنظمة الهجينة",
    "ديزل": "الأنظمة الهجينة",
    "مولد": "الأنظمة الهجينة",
    "سعر": "الأسعار والتكاليف",
    "أسعار": "الأسعار والتكاليف",
    "تكلفة": "الأسعار والتكاليف",
    "استرداد": "الأسعار والتكاليف",
    "إنفرتر": "المعدات والمكونات",
    "انفرتر": "المعدات والمكونات",
    "ألواح": "المعدات والمكونات",
    "لوح": "المعدات والمكونات",
    "تركيب": "التصميم والتركيب",
    "تصميم": "التصميم والتركيب",
    "حساب المقاس": "التصميم والتركيب",
    "مستشفى": "قطاعات ومنشآت",
    "مصنع": "قطاعات ومنشآت",
    "اتصالات": "قطاعات ومنشآت",
    "صيانة": "التشغيل والصيانة",
    "تنظيف": "التشغيل والصيانة",
    "أعطال": "التشغيل والصيانة",
}
...
def make_nav_title(title):
    ...
    nav = re.sub(r'\s*[-–—]\s*شمس النيل للطاقة.*$', '', nav)
    nav = re.sub(r'\s*\(20\d{2}.*?\)', '', nav)
    nav = re.sub(r'\s*بورتسودان:?\s*', ' ', nav).strip()
    if len(nav) <= 30:
        return nav
    words = nav.split()
    result = ''
...
def clean_body_links(body):
    body = re.sub(r'https?://shamsalnil\.com/', '/', body)
...
def convert_and_write(draft_path, output_path, publish_date, slug):
    ...
    canonical = f'https://shamsalnil.com/blog/{slug}/'
